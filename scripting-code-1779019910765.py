...
def get_password():
    ...
    while True:
        ...
        # Using list comprehensions and built-in methods
        ...
        print("\n  Password Analysis:")
        print(f"     {'YES' if has_lower  else 'NO '}  Lowercase letters  (a-z)")
        print(f"     {'YES' if has_upper  else 'NO '}  Uppercase letters  (A-Z)")
        print(f"     {'YES' if has_digit  else 'NO '}  Numbers            (0-9)")
        print(f"     {'YES' if has_symbol else 'NO '}  Symbols            (!@#$%^&*...)")
...
def build_charset(pw):
    ...
    It extracts only the pools that match the user's password composition,
    which is essential for calculating accurate mathematical combinations.
    """
    charset = set()

    if any(c.islower() for c in pw):
        charset.update("abcdefghijklmnopqrstuvwxyz")

    if any(c.isupper() for c in pw):
        charset.update("ABCDEFGHIJKLMNOPQRSTUVWXYZ")

    if any(c.isdigit() for c in pw):
        charset.update("0123456789")
...
def suggest_strong_password(label):
    ...
    print("=" * 52)
    print("  Pick one, or use it as inspiration!")
...
def brute_force_demo(password, charset):
    ...
    if pw_len > 5:
        ...
        print("  Demo only runs on passwords <= 5 chars (would take too long).")
        return
